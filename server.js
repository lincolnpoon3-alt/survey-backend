...
async function createFeishuRecord(data) {
    ...
    console.log('   [DEBUG] result.data?.record:', JSON.stringify(result.data?.record));
    console.log('   [DEBUG] result.data?.record?.record_id:', result.data?.record?.record_id);
    console.log('   [DEBUG] result.data?.record?.id:', result.data?.record?.id);
    ...
    if (!recordId) {
        console.error('   ❌ WARNING: recordId is still undefined! Response structure may have changed.');
    }
