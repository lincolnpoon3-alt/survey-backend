...
async function createFeishuRecord(data) {
    ...
    // Extract record_id from response - multiple fallback paths
    let recordId = null;
    if (result.data && result.data.record) {
        recordId = result.data.record.record_id || result.data.record.id;
    }
    if (!recordId && result.data) {
        recordId = result.data.record_id || result.data.id;
    }
    
    console.log('   ✅ Extracted recordId:', recordId);
    console.log('   📋 Full data structure:', JSON.stringify(result.data));
    
    if (!recordId) {
        console.error('   ❌ ERROR: Could not extract recordId from response!');
    }
